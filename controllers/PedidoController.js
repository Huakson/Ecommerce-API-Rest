...
const { calcularFrete } = require("./integraçoes/correios");
...
class PedidoController {
    ...
    async indexAdmin(req, res, next) {
        ...
        try {
            const pedidos = await Pedido.paginate({ loja }, { offset: Number(offset || 0), limit: Number(limit || 0), populate: ["cliente", "pagamento", "entrega"] });
    ...
    async removeAdmin(req, res, next) {
        try {
            ...
            const cliente = await Cliente.findOne({ _id: pedido.cliente, loja: pedido.loja }).populate({ path: "usuario", select: "-salt -hash" });
 
            console.log(`clientteeeee ${cliente}`);

            EmailController.cancelarPedido({ usuario: cliente.usuario, pedido });
    ...
    async index(req, res, next) {
        ...
        try {
            ...
            const pedidos = await Pedido.paginate({ loja, cliente: cliente._id }, { offset: Number(offset || 0), limit: Number(limit || 0), populate: ["cliente", "pagamento", "entrega"] });
            pedidos.docs = await Promise.all(pedidos.docs.map(async (pedido) => {
                pedido.carrinho = await Promise.all(pedido.carrinho.map(async (item) => {
                    item.produto = await Produto.findById(item.produto);
                    item.variacao = await Variacao.findById(item.variacao);
